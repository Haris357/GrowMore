...
class GeoBusinessScraper(BaseScraper):
    def __init__(self):
        super().__init__(
            source_name="Geo News Business",
    ...
    async def scrape(self) -> List[Dict[str, Any]]:
        ...
        for section in self.sections:
            ...
            article_elements = soup.select(".story, .news-item, article")
    ...
    def parse_item(self, item: Any) -> Optional[Dict[str, Any]]:
        try:
            title_elem = item.select_one("h2 a, h3 a, .story-title a, a.open-section")
            if not title_elem:
                return None
            ...
            summary_elem = item.select_one(".story-summary, .news-text, p")
            summary = self.clean_text(summary_elem.get_text()) if summary_elem else ""

            date_elem = item.select_one(".story-date, time, .date-time")
            published_at = None
            if date_elem:
            ...
            logger.error(f"Error parsing Geo article: {e}")
            return None
    ...
    async def scrape_article_content(self, url: str) -> Optional[str]:
        ...
        soup = self.parse_html(html)
        content_elem = soup.select_one(".content-area, .story-detail")
