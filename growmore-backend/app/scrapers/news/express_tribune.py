"""
Express Tribune Business news scraper.
Source: tribune.com.pk/business
"""
import logging
...
class ExpressTribuneScraper(BaseScraper):
    def __init__(self):
        super().__init__(
    ...
    async def scrape(self) -> List[Dict[str, Any]]:
        ...
        for section in self.sections:
            ...
            article_elements = soup.select(".story-article, .story, .article-item, .news-item")
    ...
    def parse_item(self, item: Any) -> Optional[Dict[str, Any]]:
        try:
            title_elem = item.select_one("h2 a, h3 a, .story-title a, .title a")
            if not title_elem:
                return None
            ...
            summary_elem = item.select_one(".story-excerpt, .excerpt, .description, p")
            summary = self.clean_text(summary_elem.get_text()) if summary_elem else ""

            date_elem = item.select_one(".story-date, .date, time, .timestamp")
            published_at = None
            if date_elem:
            ...
            author_elem = item.select_one(".author, .story-author, .byline")
            ...
            logger.error(f"Error parsing Express Tribune article: {e}")
            return None
    ...
    async def scrape_article_content(self, url: str) -> Optional[str]:
        ...
        soup = self.parse_html(html)
        content_elem = soup.select_one(".story-content, .article-content, .body-content")
