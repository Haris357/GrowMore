...
class DawnBusinessScraper(BaseScraper):
    def __init__(self):
        super().__init__(
            source_name="Dawn Business",
    ...
    async def scrape(self) -> List[Dict[str, Any]]:
        ...
        for section in self.sections:
            ...
            article_elements = soup.select("article, .story, .template__main article")
    ...
    def parse_item(self, item: Any) -> Optional[Dict[str, Any]]:
        try:
            title_elem = item.select_one("h2 a, h3 a, .story__title a")
            if not title_elem:
                return None
            ...
            summary_elem = item.select_one(".story__excerpt, .story__summary")
            summary = self.clean_text(summary_elem.get_text()) if summary_elem else ""

            date_elem = item.select_one(".timestamp__date, time, .story__time")
            published_at = None
            if date_elem:
            ...
            author_elem = item.select_one(".story__byline, .author")
            ...
            logger.error(f"Error parsing Dawn article: {e}")
            return None
    ...
    async def scrape_article_content(self, url: str) -> Optional[str]:
        ...
        soup = self.parse_html(html)
        content_elem = soup.select_one(".story__content, .template__content")
